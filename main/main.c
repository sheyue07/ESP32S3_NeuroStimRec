...
static bool gpio7_level_is_stable(bool expected_high)
{
    ...
    while ((xTaskGetTickCount() - start_tick) < stable_ticks) {
        const bool high = gpio_get_level(RECORD_SWITCH_GPIO) == 1;
...
static void sd_write_task(void *parameter)
{
    ...
    while (true) {
        const bool switch_high = gpio_get_level(RECORD_SWITCH_GPIO) == 1;
        ...
        if (!currently_recording) {
            ...
            const esp_err_t start_result = continuous_rx_start();
            if (start_result != ESP_OK) {
                signal_capture_failure("Continuous receiver start", start_result);
                close_failed_start(&context);
                start_armed = false;
                continue;
            }
            ...
            start_armed = false;
            last_log_time_us = esp_timer_get_time();
            ESP_LOGI(TAG,
                     "SYNC SEARCH STARTED: header=FFFF0000, frame=%u bytes",
                     ADC_FRAME_SIZE_BYTES);
            continue;
        }
        ...
        get_capture_status(&status);
        if (status.failed) {
            stop_pipeline_drain_and_finish(&context, "capture failure");
            currently_recording = false;
            continue;
        }
        ...
        log_write_rate(&context, &last_log_time_us);
    }
}
...
void app_main(void)
{
    ...
    if (recording_mutex == NULL || dma_stopped_sem == NULL ||
