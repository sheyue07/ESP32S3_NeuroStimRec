...
| GPIO5 | `Stim_ctrl` | 输入 | 高电平请求启动刺激，低电平请求停止刺激 |

GPIO5 配置下拉，按 100 us 稳定时间消抖。不得把 GPIO5 和 GPIO7 合并。
...
- GPIO5发生变化后启动或重启一次 100 us 单次定时。
- 定时到期再次读取GPIO5；电平与候选状态一致才提交新状态。
...
GPIO5稳定变高后按以下顺序发送10帧。右侧给出应当生成的完整5字节结果，
可直接作为单元测试黄金值：
...
GPIO5在启动序列中途变低时，应在当前40位帧和所属101样本槽完成后切换到
...
- `stim_controller`：GPIO5配置、100 us消抖、状态转换和故障安全处理。
...
- 日志至少包含初始化频率、所用时钟分频参数、GPIO5稳定状态转换、启动序列
  完成、停止状态和任何DMA故障。
...
- 更新README的硬件引脚表、GPIO5/GPIO7职责和刺激状态说明。
...
- GPIO5低时重复停止帧；稳定高后只发送一次10帧序列；再次低后恢复停止帧。
