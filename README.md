...
首次采集前仍可扫描和导出上次留在 eMMC 中的数据。固件用 RTC 保留区分同一次供电周期：真正断电后，下一次采集会建立新 run 并从 LBA 2048 覆盖；串口重连、RESET、软件复位或 `REINIT` 后则读取冗余超级块和分段目录，从上一分段末尾继续写。若复位发生在采集中，最近一次 64 MiB 检查点之前的完整帧会保留，该 OPEN 分段会标成失败，后续采集从其后开始。
...
实时预览从 GPIO20/16 接收后的已验证 260 字节帧中提取，不读取 eMMC。为避免干扰 CPU0 帧同步，预览只在 CPU1 存储任务接收一批有效帧后按目标频率跳选，不再逐帧调用预览代码。BLE 使用 `0x91 PREVIEW_BATCH` 按协商 ATT MTU 动态聚合多个时间点，并将等待上限限制为 500 ms；50 点/秒、两个通道时通常由约 50 次通知/秒降为约 2 次通知/秒，八个通道约为 6–7 次通知/秒。预览队列满时仅增加预览丢弃计数；完整帧仍按原路径写入 eMMC。
...
手机预览默认仍为约 50 点/秒，但多个时间点会合并为批量通知；关闭预览不影响采集、刺激和状态控制。预览只用于低频趋势观察；超过 25 Hz 的输入会混叠，不能据此判断高频波形是否失真。完整 14.4 kframe/s 左右的有效帧仍写入 eMMC，应以导出的 BIN/MAT 数据验证 1 kHz 等高频信号。
...
2026-08-30 对采集热路径与 BLE 内存配置完成调整并通过 ESP-IDF 6.0.2 全量编译：静态 DIRAM 使用约 96.7 KiB，固件镜像约 614 KiB。ADC 接收 GDMA 使用 ESP32-S3 支持的 64 字节突发和最高通道优先级，并检查此前未统计的 SPI RX FIFO 溢出；检测到时返回 `0x7113`，不再让字节缺口最终被误报为原始环形缓冲溢出。该构建仍需在真实硬件上完成“BLE 扫描连接—至少 60 秒采集—停止—扫描分段—导出”的回归验证。

2026-09-01 `r4-stop-safe-dma-sync` 将正常停止与运行中故障分开：停止请求先进入
`stopping` 状态，GDMA 收尾产生的半描述符或 EOF 不再把完整分段误标为
`FAILED_PIPELINE`。运行中真实发生的描述符、环形块覆盖、完成队列、意外 EOF 和
块所有权故障分别使用 `0x7114`～`0x7118`，并持久化到分段目录。
...
- 多种管线错误仍统一归类为结果码 5，但分段目录同时持久化具体 `failure_code`；应结合该错误码、`STATUS`、目录和事件数据定位。
- 输入信号错误率过高会让同步器长期运行在较慢的全局搜索路径，7 MiB 原始环形缓冲仍可能被持续积压耗尽并触发结果码 5。
